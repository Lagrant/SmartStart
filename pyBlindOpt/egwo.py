...
cached=False, debug=False, verbose=False, seed:int=42) -> tuple:
    ...
    scores = utils.vectorized_evaluate(pop, objective_cache)
    alfa_score, beta_score, gamma_score = sorted(scores)[0:3]
    alfa_wolf = pop[np.where(scores == alfa_score)[0][0]]
    beta_wolf = pop[np.where(scores == beta_score)[0][0]]
    gamma_wolf = pop[np.where(scores == gamma_score)[0][0]]
    ...
    for epoch in tqdm.tqdm(range(n_iter), disable=not verbose):
        ...
        scores_offspring = utils.vectorized_evaluate(offspring, objective_cache)
        for i in range(n_pop):
